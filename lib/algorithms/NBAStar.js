...
export class NBAStar extends AStar {
    ...
    async findPath(from, to) {
        ...
        const fetchPromises = [];
        if (!this.NG.has(from.id)) {
            fetchPromises.push(this.fetchNodeTile(from));
        }
        if (to && !this.NG.has(to.id)) {
            fetchPromises.push(this.fetchNodeTile(to));
        }
        ...
        pool.set(FROM.id, {
            ...
            fFScore: this.heuristic(FROM, TO),
        ...
        pool.set(TO.id, {
            ...
            bFScore: this.heuristic(TO, FROM),
            fIndex: -1,
            bIndex: -1
        });
    ...
    async visitNeighbors(FROM, TO, cameFrom, pool, queue, forward) {
        ...
        const searchSource = forward ? FROM : TO;
        const searchTarget = forward ? TO : FROM;

        // Condition to determine if a neighbor node is stabilized or rejected
        if (nFScore < this.L && (cfGScore + fScore - this.heuristic(cameFromNG, searchSource)) < this.L) {
            for (const neighborId of forward ? cameFromNG.nextNodes : cameFromNG.prevNodes) {
                ...
                /**
                     ...
                     * or the tile it belongs to is not cached. So get on it and fetch the tile!
                */
                ...
                const tGScore = cfGScore + this.distance(neighborNG, cameFromNG);
                ...
                if (!neighbor || tGScore < nGScore) {
                    ...
                    if (pool.has(neighborId)) {
                        if (forward) {
                            ...
                            neighbor.fFScore = tGScore + this.heuristic(neighborNG, searchTarget);
                            neighbor.previous = cameFrom;
                        } else {
                            neighbor.bGScore = tGScore;
                            neighbor.bFScore = tGScore + this.heuristic(neighborNG, searchTarget);
                        ...
                        if (forward) {
                            pool.set(neighborId, {
                                ...
                                fGScore: tGScore,
                                bGScore: Number.POSITIVE_INFINITY,
                                fFScore: tGScore + this.heuristic(neighborNG, searchTarget),
                                bFScore: Number.POSITIVE_INFINITY,
                                previous: cameFrom,
                                fIndex: -1,
                                bIndex: -1
                            ...
                            pool.set(neighborId, {
                                ...
                                bGScore: tGScore,
                                fFScore: Number.POSITIVE_INFINITY,
                                bFScore: tGScore + this.heuristic(neighborNG, searchTarget),
                                next: cameFrom,
                                fIndex: -1,
                                bIndex: -1
                            });
    ...
    calculatePathCost(meetingPoint) {
        ...
        let previous = meetingPoint;
        while (previous.previous) {
            cost += this.distance(this.NG.get(previous.id), this.NG.get(previous.previous.id));
            previous = previous.previous;
        }

        let next = meetingPoint;
        while (next.next) {
            cost += this.distance(this.NG.get(next.next.id), this.NG.get(next.id));
            next = next.next;
        }
        return cost;
    }
