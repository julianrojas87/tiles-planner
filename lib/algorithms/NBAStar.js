...
export class NBAStar extends AStar {
    ...
    async findPath(from, to) {
        ...
        while (forwardQueue.length > 0 && backwardQueue.length > 0) {
            ...
            if (this.M) return this.rebuildPath();
    ...
    async visitNeighbors(FROM, TO, cameFrom, pool, queue, forward) {
        ...
        const nGScore = forward ? cameFrom.fGScore : cameFrom.bGScore;
        ...
        if (nFScore < this.L && (nGScore + fScore - this.heuristic(cameFromNG.coordinates, searchSource)) < this.L) {
            for (const neighborId of forward ? cameFromNG.nextNodes : cameFromNG.prevNodes) {
                ...
                */
                ...
                const tGScore = nGScore + this.distance(neighborNG);
                ...
                if (!neighbor || tGScore < nGScore) {
                    ...
                    if (index < 0) {
                        // Add to the queue
                        queue.push(neighbor);
                        this.logger.debug("(%s) Queued %s with fScore %s", 
                            this.searchLabel(forward), neighborId, forward ? neighbor.fFScore : neighbor.bFScore);
                    } else {
                        // Update on the queue
                        queue.updateItem(neighbor, index);
                        this.logger.debug("(%s) Updated (had index %s) %s in queue with fScore %s", 
                            this.searchLabel(forward), index, neighborId, forward ? neighbor.fFScore : neighbor.bFScore);
                    }

                    // Check if this is a meeting point by checking if we know 
                    // both forward and backward G scores.
                    const potentialMin = neighbor.fGScore + neighbor.bGScore;
                    if (potentialMin < this.L) {
                        this.M = neighbor;
                    }
                }
            }
    ...
    rebuildPath() {
        const path = [this.NG.get(this.M.id)];
